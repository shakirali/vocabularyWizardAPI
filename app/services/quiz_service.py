...
from app.utils.quiz_generator import generate_quiz_questions, generate_sentence_questions
from app.schemas.quiz import GenerateQuizRequest, SubmitQuizRequest, GenerateSentenceRequest, SubmitSentenceRequest
import uuid
...
class QuizService:
    ...
    def generate_quiz(self, user_id: uuid.UUID, request: GenerateQuizRequest) -> dict:
        ...
        mastered_ids = self.progress_repo.get_mastered_word_ids_by_year(user_id, request.year)
    ...
    def generate_sentences(self, user_id: uuid.UUID, request: GenerateSentenceRequest) -> dict:
        """Generate sentence fill-in-the-blank questions."""
        # Get all vocabulary items for the year (not just mastered)
        vocabulary_items, _ = self.vocab_repo.get_all_with_filters(year=request.year, limit=1000)
        ...
        questions = generate_sentence_questions(vocabulary_items, request.question_count)
    ...
    def submit_sentences(self, user_id: uuid.UUID, request: SubmitSentenceRequest) -> dict:
